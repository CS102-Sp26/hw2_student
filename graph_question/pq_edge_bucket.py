"""
pq_edge_bucket.py
Starter code for the Priority Queue variant of The Bucket Algorithm where Priority Queue of edges and their weights is maintained and minimum is dequed while selecting an edge which results in a new vertex to be brought inside the bucket.
"""
...
def pq_edge_bucket(graph, start_vertex):
    '''
    ...
    for i in range(n):
        if not visited[i]:
            ...
            if first:
                start = start_vertex
                first = False
            bucket = pq_edge_bucket(graph, start)
