"""
pq_dist_bucket.py
Reference solution for the Priority Queue variant of The Bucket Algorithm where Priority Queue of vertices and their distances is maintained and minimum is dequed while selecting new vertex to be brought inside the bucket.
"""
...
def pq_dist_bucket(graph, start_vertex):
    '''
    ...
    for i in range(n):
        if not visited[i]:
            ...
            if first:
                start = start_vertex
                first = False
            bucket = pq_dist_bucket(graph, start)
