"""
basic_bucket.py
Starter code for The Bucket Algorithm
"""
...
def basic_bucket(graph, start_vertex):
    '''
...
def all_buckets(graph, start_vertex):
    '''
        runs the bucket algorithm for all components
        and returns list of sizes of all buckets
        if the graph is connected, there will be one number in the list whose value
    ...
    for i in range(n):
        if not visited[i]:
            ...
            if first:
                start = start_vertex
                first = False
            bucket = basic_bucket(graph, start)
...
def main(input_path):
    ...
    bucket_sizes = all_buckets(graph, start_vertex)
